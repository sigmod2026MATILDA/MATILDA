...
class Combiner:
        ...
        while True:
            ...
            with solver.solve(yield_=True) as handle:
                ...
                while True:
                    ...
                    if (
                        not self.settings.recursion_enabled
                        and not self.settings.pi_enabled
                    ):
                        best_prog = rules
                        best_fn = fn
                        if not self.solution_found:
                            # FIX:
                            try:
                                self.pos_covered = self.tester.get_pos_covered(
                                    model_prog
                                )
                    ...
                    if not model_inconsistent:
                        ...
                        if fn > 0 and len(self.pos_covered) == len(
                            self.settings.pos_index
                        ):
                            best_fn = 0
                        continue
                    ...
                    con = (
                        ":-"
                        + ",".join(
                            f"rule({self.rulehash_to_id[get_rule_hash(rule)]})"
                            for rule in smaller
                        )
                        + "."
                    )
    ...
    def build_encoding2(self):
        ...
        if self.solution_found:
            ...
            this_encoding.add(
                ":- #sum{K,R : rule(R), size(R,K)} >= " + f"{self.max_size}."
            )
        else:
    ...
    def update_best_prog(self, prog, pos_covered):
        ...
        self.settings.print_incomplete_solution(
            new_solution, self.tester.num_pos, 0, size
        )
