...
def explain_inconsistent(
    settings, generator, tester, prog, rule_ordering, new_cons, all_handles
):
        ...
        with generator.solver.solve(yield_=True) as handle:
            ...
            while True:
                ...
                if (
                    ...
                    and num_pos_covered > 0
                    and tester.is_non_functional(prog)
                ):
                            ...
                            if tester.has_redundant_literal([rule]):
                                ...
                                if len(prog) > 1:
                                    (
                                        new_handles,
                                        con,
                                    ) = generator.build_generalisation_constraint(
                                        [rule]
                                    )
                                    new_cons.add(con)
                                    all_handles.update(
                ...
                if (
                    settings.recursion_enabled
                    and settings.max_rules > 2
                    and is_recursive
                ):
                ...
                if (
                    settings.recursion_enabled
                    and len(prog) > 2
                    and tester.has_redundant_rule(prog)
                ):
                    add_gen = True
                ...
                if not settings.recursion_enabled:
                    ...
                    if (
                        not add_spec
                        and combiner.solution_found
                        and num_pos_covered == 1
                    ):
                        add_spec = True
                    ...
                    if combiner.solution_found:
                        for x in seen_covers_only_one_gen:
                            (
                                new_handles,
                                con,
                            ) = generator.build_generalisation_constraint(x)
                            new_cons.add(con)
                            all_handles.update(parse_handles(generator, new_handles))
                        ...
                        for x in seen_covers_only_one_spec:
                            (
                                new_handles,
                                con,
                            ) = generator.build_specialisation_constraint(x)
                            new_cons.add(con)
                            all_handles.update(parse_handles(generator, new_handles))
                        seen_covers_only_one_spec = set()
                ...
                if add_gen and not pruned_sub_inconsistent:
                    if (
                        settings.recursion_enabled
                        or settings.pi_enabled
                        or not pruned_sub_incomplete
                    ):
...
def learn_solution(settings):
    timeout(
        ...
        popper,
        (settings,),
        timeout_duration=600000000#int(settings.timeout),
    )
